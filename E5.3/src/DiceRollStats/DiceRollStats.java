...
public class DiceRollStats {
   ...
   public static void main(String[] args) {
       double[] tabStats;  // The average number of rolls to get a given total.
       System.out.println("Total On Dice     Average Number of Rolls	 Standard deviation		     Max");
       System.out.println("-------------     -----------------------     -----------------------     -----------------------");
       for ( int dice = 2;  dice <= 12;  dice++ ) {
          tabStats = getAverageRollCount( dice );
          System.out.printf("%10d%22.4f%29.4f%27.0f\n", dice, tabStats[0], tabStats[1], tabStats[2]);
   ...
   public static double[] getAverageRollCount( int roll ) {
	  ...
	  double tabStat[] = new double[3];
	  
      int rollCountThisExperiment;  // Number of rolls in one experiment.
      for ( int i = 0;  i < NUMBER_OF_EXPERIMENTS;  i++ ) {
          rollCountThisExperiment = rollFor( roll );
          stat.enter(rollCountThisExperiment);
          
      }
      tabStat[0] = stat.getMean();
      tabStat[1] = stat.getStandardDeviation();
      tabStat[2] = stat.getMax();
